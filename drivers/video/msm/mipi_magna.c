...
static int mipi_magna_lcd_on(struct platform_device *pdev)
{
	...
	printk("[LIVED] mipi_magna_lcd_on ===> \n");
...
int mipi_magna_lcd_off_State=0;
...
static int mipi_magna_lcd_off(struct platform_device *pdev)
{
	...
	printk("[LIVED] mipi_magna_lcd_off disp_on=%d |===> \n",(int)magna_state.disp_on);

	mipi_magna_lcd_off_State=0;
	mfd = platform_get_drvdata(pdev);
	mipi_magna_lcd_off_State=1;

	if (!mfd){
		mipi_magna_lcd_off_State=100;
		EXIT_FUNC();
		printk("[LIVED] mipi_magna_lcd_off Error mfd is NULL <===| \n");
		mipi_magna_lcd_off_State=101;
		return -ENODEV;
	}
	if (mfd->key != MFD_KEY){
		mipi_magna_lcd_off_State=200;
		EXIT_FUNC();
		printk("[LIVED] mipi_magna_lcd_off Error mfd->key != MFD_KEY <===| \n");
		mipi_magna_lcd_off_State=201;
		return -EINVAL;
	}

	mipi_magna_lcd_off_State=2;
	if (magna_state.disp_on == true) {
		mipi_magna_lcd_off_State=3;
		gpio_set_value(LCD_RESET, GPIO_LOW_VALUE);
		mipi_magna_lcd_off_State=4;
		msleep(1);
		mipi_magna_lcd_off_State=5;
		//msleep(15);
		gpio_set_value(LCD_RESET, GPIO_HIGH_VALUE);
		mipi_magna_lcd_off_State=6;
	    msleep(5);
		mipi_magna_lcd_off_State=7;

		//mutex_lock(&mfd->dma->ov_mutex);
#if 1	//[BIH] workaround LCD not wake-up problem
		mipi_magna_lcd_off_State=8;
		printk("%s mipi_dsi_cmds_tx |===>\n",__func__);
		...
		mipi_magna_lcd_off_State=10;

#ifdef CONFIG_SKY_CHARGING
                msm_charger_set_lcd_onoff(false);
#endif
		mipi_magna_lcd_off_State=11;
...
#endif
	printk("[LIVED] mipi_magna_lcd_off Done <===| \n");

	mipi_magna_lcd_off_State=20;
	return 0;
}
