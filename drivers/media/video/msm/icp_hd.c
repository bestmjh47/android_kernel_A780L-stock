...
#define ICP_HD_I2C_RETRY	15 //5//10
#define ICP_HD_I2C_MPERIOD	200 //30//200
...
#if 1 //This is a test register whether device is alive or not
	icp_hd_i2c_read(icp_hd_client->addr, 0x0000, &chip_id,WORD_LEN);
...
//	CAM_INFO("SENSOR REVISION NUM~~~ = 0x%x\n", revision_num);
    ...
    if(chip_id == 0x0000)
    {
      for(i=0; i<500; i++)
	  {
		...
		if(chip_id == 0xec1)
		{
		  CAM_ERR("icp_hd_reg_init() - RETRY SENSOR CHIP ID~~~ = 0x%x\n", chip_id);
          break;
		}
      }
    }	
