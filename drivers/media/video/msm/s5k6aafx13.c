...
#define S5K6AAFX13_I2C_RETRY	15
	 ...
	 unsigned char *txdata, int length)
{
	...
	for (i = 0; i < S5K6AAFX13_I2C_RETRY; i++) {
		...
		CAM_ERR("%s: tx retry. [%02x.%02x.%02x] len=%d rc=%d\n", __func__,saddr, *txdata, *(txdata + 1), length, rc);
...
static int32_t icp_hd_i2c_read(unsigned short   saddr,
	unsigned short raddr, unsigned int *rdata, enum s5k6aafx13_width width)
{
	int32_t rc = 0;
	unsigned char buf[4];
...
//	int8_t m_wb = 0;
    ...
    if(whitebalance < 0 || whitebalance > 6){
        CAM_ERR("%s error. whitebalance=%d\n", __func__, whitebalance);
        return -EINVAL;
    }
...
{
#ifdef F_ICP_HD_STANDBY
#if 1 //This is a test register whether device is alive or not
...
#ifdef F_ICP_HD_STANDBY
#if 1 //This is a test register whether device is alive or not
		icp_hd_i2c_read(ICP_SLAVE_ADDRESS, 0x0000, &chip_id,TRIPLE_LEN);
		CAM_INFO("SENSOR CHIP ID~~~ = 0x%x\n", chip_id);
//		icp_hd_i2c_read(ICP_SLAVE_ADDRESS, 0x0050, &revision_num,WORD_LEN);
//		CAM_INFO("SENSOR REVISION NUM~~~ = 0x%x\n", revision_num);
		...
		if(chip_id == 0x0000)
		{
		  for(i=0; i<500; i++)
		  {
			...
			if(chip_id == 0xec1)
			{
			  CAM_ERR("s5k6aafx13_sensor_init() - RETRY SENSOR CHIP ID~~~ = 0x%x\n", chip_id);
			  break;
			}
		  }
		}	
